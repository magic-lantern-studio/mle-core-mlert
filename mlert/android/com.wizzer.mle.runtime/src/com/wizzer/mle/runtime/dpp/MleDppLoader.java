...
public class MleDppLoader
{
        ...
        throws MleDppException, MleRuntimeException
    {
        ...
        IMleRandomAccess file = MleTitle.getInstance().m_theDpp.getFile();
        ...
        throws MleDppException, MleRuntimeException
    {
        ...
        // XXX -- Fix when DPP API is available.
        // Get the Set offset from the DPP TOC. Then read the length for the associcated
        // Set.
        int length;
        try
        {
    ...
    public MleDppInput mleLoadPlayprint(String filename,long type)
    {
        ...
        try
        {
            ...
            if (pp != null)
            {
            	// Initialize the Playprint.
            	status = pp.begin((int)type);
            	if (status == true)
            	{
            		// Read the Playprint header.
            		status = pp.readHeader(null,null);
            		if (status == true)
            		{
            			// Read the Playprint Table-Of-Contents.
            			status = pp.readTOC(null,null);
            			if (status != true)
            				pp = null;
            		} else
            			pp = null;
     ...
     * Load the Digital Playprint.
     * 
     * @param stream The Digital Playprint randome-access stream.
     * @param type The type of Digital Playprint to load.
     * 
     * @return If the Digital Playprint is successfully loaded, then a reference
     * to a <code>MleDppInput</code> is returned. If the DPP fails to load, then <b>null</b>
     * will be returned.
     */
    public MleDppInput mleLoadPlayprint(InputStream stream,long type)
    {
        MleDppInput pp = null;
        boolean status = false;
