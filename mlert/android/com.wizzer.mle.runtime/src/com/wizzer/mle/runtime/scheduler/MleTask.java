...
/**
 * <code>MleTask</code> is a class that manages a named unit of execution.
 * <p>
 * MleTask encapsulates an autonamous action for the Magic Lantern scheduler,
 * <code>MleScheduler</code>. By default, the task will
 * run in a synchronous manner. However, the task may be run asynchronously using the
...
public class MleTask extends Object
{
    ...
    // A flag indicating whether to use a thread or not upon invokation;
    private boolean m_useThread = false;
    ...
    /**
     * Determine whether to use a <code>Thread</code> for execution or not.
     * 
     * @param state If <b>true</b>, then a thread will be used and the task
     * will run asynchronously.
     * Otherwise, if <b>false</b>, then the task will run synchronously.
     */
    public void useThread(boolean state)
    {
    	m_useThread = state;
    }
